...
const Bookmarks = () => {
  ...
  return (
    <AppLayout
      variant="wide"
      right={
        <div className="vk-card p-2 flex flex-col gap-0.5 overflow-y-auto">
    ...
    >
      {/* Заголовок и поиск */}
      <div className="vk-card p-4 flex flex-col gap-3">
      ...
      <div className="vk-card p-2 max-h-[calc(100vh-220px)] overflow-y-auto flex flex-col gap-1">
        {visible.length === 0 ? (
          <EmptyState />
        ) : (
          visible.map((item) => (
...
const BookmarkRow = ({
  ...
  return (
    <div className="group flex items-center gap-3 p-2 rounded-xl hover:bg-secondary/60 transition-colors">
      <div
        className={`w-14 h-14 rounded-xl bg-gradient-to-br ${item.accent} flex items-center justify-center shrink-0`}
      >
...
const EmptyState = () => (
  <div className="min-h-[320px] flex items-center justify-center p-10">
