...
const Market = () => {
  ...
  const TopActions = (
    <div className="vk-card flex items-center gap-3 p-3">
      <button onClick={() => setView({ kind: "catalog" })} className="vk-pill flex items-center gap-2">
      ...
      <button onClick={() => setView({ kind: "cart" })} className="vk-pill flex items-center gap-2 relative">
    ...
    <>
      <div className="vk-card p-2">
        {["Женщинам", "Мужчинам", "Детям"].map(s => (
          <button key={s} className="vk-nav-item w-full justify-start">{s}</button>
        ))}
      </div>
  ...
  const renderCatalog = () => (
    <>
      ...
      <div className="vk-card p-3 flex items-center gap-2 flex-wrap">
  ...
  const renderCategory = (category: string) => {
    ...
    return (
      <>
        {TopActions}
        <div className="vk-card p-4 flex items-center gap-2 text-sm">
          ...
          <button className="ml-auto vk-pill bg-transparent flex items-center gap-1.5 text-muted-foreground">
            <MapPin className="w-3.5 h-3.5" /> Bucureşti
          </button>
        </div>
        <div className="vk-card p-4">
  ...
  function ProductCard({ p }: { p: Product }) {
    ...
    return (
      <div className="group cursor-pointer" onClick={() => setView({ kind: "product", id: p.id })}>
        <div className={`relative aspect-square rounded-xl bg-gradient-to-br ${p.grad} overflow-hidden`}>
  ...
  const renderProduct = (id: string) => {
    ...
    if (!p) return <div className="vk-card p-6">Товар не найден</div>;
      ...
      <>
        ...
        <div className="vk-card p-4">
          <div className="grid grid-cols-[80px_minmax(0,1fr)_360px] gap-4">
            ...
            <div className="flex flex-col gap-2">
              {[0, 1, 2].map(i => (
                <div key={i} className={`aspect-square rounded-lg bg-gradient-to-br ${p.grad} ${i === 0 ? "ring-2 ring-primary" : "opacity-70"}`} />
              ))}
            </div>
            {/* main image */}
            <div className={`relative rounded-xl bg-gradient-to-br ${p.grad} aspect-square w-full`} />
            {/* details */}
            <div className="flex flex-col">
              <div className="text-2xl font-bold">{fmt(p.price)}</div>
              {p.views !== undefined && (
                <button className="vk-pill self-start mt-2 bg-secondary text-xs flex items-center gap-1.5">
              ...
              <button onClick={() => setView({ kind: "seller", seller: p.seller })}
                ...
                <div className="w-10 h-10 rounded-full bg-gradient-to-br from-primary to-purple-600 shrink-0" />
        ...
        <div className="vk-card p-4">
          ...
          <div className="text-center py-10">
            ...
            <button className="vk-pill mt-3 bg-secondary">Оставить отзыв</button>
          </div>
        </div>
        ...
        <div className="vk-card p-4">
          ...
          <div className="flex items-center gap-3">
            <div className="w-12 h-12 rounded-full bg-gradient-to-br from-primary to-purple-600 shrink-0" />
            ...
            <button className="vk-pill bg-secondary">Подписаться</button>
            <button onClick={() => setView({ kind: "seller", seller: p.seller })} className="vk-pill bg-secondary">Перейти в магазин</button>
          </div>
        </div>
  ...
  const renderSeller = (seller: string) => {
    ...
    return (
      <>
        {TopActions}
        <div className="vk-card p-4 flex items-center gap-3">
          <div className="w-14 h-14 rounded-full bg-gradient-to-br from-primary to-purple-600 shrink-0" />
          <div className="flex-1 min-w-0">
          ...
          <button className="vk-pill bg-secondary">Подписаться</button>
          <button onClick={() => sample && setMessageProduct(sample)} className="vk-pill bg-secondary flex items-center gap-1.5">
        ...
        <div className="vk-card p-4">
          <div className="flex items-center justify-between mb-4">
            ...
            <button className="vk-pill bg-secondary text-xs flex items-center gap-1.5">По умолчанию <ChevronDown className="w-3.5 h-3.5" /></button>
          </div>
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 2xl:grid-cols-5 gap-3">
            {list.map(p => (
              <div key={p.id}>
      ...
      <>
        ...
        {cartItems.length === 0 ? (
          <div className="vk-card p-10 text-center">
            ...
            <button onClick={() => setView({ kind: "catalog" })} className="vk-pill mt-4 bg-primary text-primary-foreground">К покупкам</button>
          </div>
        ) : Object.entries(groups).map(([seller, list]) => {
          const subTotal = list.reduce((s, { product, qty }) => s + product.price * qty, 0);
          return (
            <div key={seller} className="vk-card p-4">
              <div className="flex items-center gap-3 pb-3 border-b border-border">
                ...
                <div className="ml-auto flex items-center gap-2">
                  <button onClick={() => setMessageProduct(list[0].product)} className="vk-pill bg-secondary text-xs">Написать продавцу</button>
                  <button onClick={() => clearSeller(seller)} className="vk-pill bg-secondary text-xs">Удалить все товары</button>
                </div>
              </div>

              {list.map(({ product, qty }) => (
                <div key={product.id} className="flex items-center gap-4 py-4 border-b border-border last:border-0">
                  <div className={`w-24 h-24 rounded-lg bg-gradient-to-br ${product.grad} shrink-0 cursor-pointer`}
              ...
              <div className="pt-4 flex items-start gap-6">
                ...
                <div className="text-right">
                  ...
                  <button onClick={() => setView({ kind: "checkout" })} className="vk-pill bg-secondary hover:bg-accent">Перейти к оформлению</button>
                </div>
              </div>
            </div>
          );
    ...
    return (
      <>
        ...
        <div className="grid grid-cols-[minmax(0,1fr)_360px] gap-4">
          <div className="vk-card p-5 flex flex-col gap-5">
            <div>
            ...
            <div className="border-t border-border pt-4">
              ...
              {cartItems.map(({ product, qty }) => (
                <div key={product.id} className="flex items-center gap-3 py-2">
                  <div className={`w-16 h-16 rounded-lg bg-gradient-to-br ${product.grad} shrink-0`} />
                  <div className="flex-1 min-w-0">
          ...
          <div className="vk-card p-5 h-fit sticky top-[72px] flex flex-col gap-4">
  ...
  return (
    <AppLayout variant="wide" right={right}>
      {content}

      <Dialog open={!!messageProduct} onOpenChange={(o) => !o && setMessageProduct(null)}>
        <DialogContent className="max-w-[520px] p-0 rounded-2xl overflow-hidden bg-popover border-border">
          ...
          {messageProduct && (
            <div className="p-4 flex flex-col gap-4">
              ...
              <div className="flex items-center gap-3 p-2 rounded-lg bg-secondary/50">
                <div className={`w-12 h-12 rounded-lg bg-gradient-to-br ${messageProduct.grad} shrink-0`} />
                <div className="min-w-0">
              ...
              <div className="flex items-center gap-3">
                ...
                <button onClick={() => setMessageProduct(null)} className="vk-pill bg-secondary hover:bg-accent">Отправить</button>
              </div>
            </div>
          )}
        </DialogContent>
