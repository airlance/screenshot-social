...
import {
  ...
  Play, Pause, ChevronUp, ChevronDown, BadgeCheck, Plus, Search, Smile, Paperclip, Send, X, Music as MusicIcon, Zap,
} from "lucide-react";
...
const SUGGESTED = [
  { id: "s1", cover: postPhoto1, author: "Полина Создатель Видео", title: "Мои штаны, которые ищут все!💔 Арт..." },
  { id: "s2", cover: postPhoto2, author: "Анастасия Головина", title: "Шоколадные яйца из КБ. Апрель 2026..." },
  { id: "s3", cover: photo1, author: "Зайцева_Хоум", title: "Моя самая эстетичная покупка 🥰 ..." },
  { id: "s4", cover: postPhoto3, author: "Кристина", title: "3949₽ — стоит ли своих денег?" },
  { id: "s5", cover: postPhoto4, author: "Lays", title: "НОВЫЙ LAYS? ОПЯТЬ НОВИНКА" },
  { id: "s6", cover: story1, author: "VS", title: "кроссовки+джинсы vs туфли+джинсы" },
  { id: "s7", cover: postVideoThumb, author: "Tech Media", title: "Большой разговор про технологии" },
  { id: "s8", cover: postMusicCover, author: "ХАННА", title: "Музыка, тренды и закулисье" },
  { id: "s9", cover: avatar5, author: "Urban Lab", title: "Тайные маршруты старого города" },
];

const COMMENTS = [
  { id: 1, author: "Ольга Кузнецова", avatar: avatar2, time: "14 апр в 17:33", text: "Скоро будет иностранная фан база Мартина 😍", likes: 4 },
  { id: 2, author: "Мария Саблина", avatar: avatar3, time: "14 апр в 18:04", text: "Канешна давно", likes: 0 },
  { id: 3, author: "Saint Agatha", avatar: avatar5, time: "14 апр в 19:02", text: "хаха я помню тебя еще с Евровидения", likes: 3 },
  { id: 4, author: "Мийка Пуська", avatar: avatar6, time: "14 апр в 11:41", text: "ХА ХА 🦊🦊", likes: 12 },
  { id: 5, author: "Вера Вера", avatar: avatar7, time: "14 апр в 11:32", text: "Вам прям идёт 🤣", likes: 2 },
];
...
const ClipPlayer = ({ clip, muted, onToggleMute, playing, onTogglePlay, onPrev, onNext }: {
  ...
  <div className="flex gap-4">
    ...
    <div className="relative bg-black rounded-2xl overflow-hidden" style={{ width: 360, aspectRatio: "9/16" }}>
      <img src={clip.cover} alt={clip.description} className="absolute inset-0 w-full h-full object-cover" />
...
const ClipInfoCard = ({ clip }: { clip: Clip }) => (
  <div className="vk-card p-4 rounded-xl">
    <div className="flex items-start gap-3">
      <img src={clip.authorAvatar} alt={clip.author} className="w-10 h-10 rounded-full object-cover" />
...
const CommentsCard = ({ count }: { count: number }) => (
  <div className="vk-card rounded-xl flex flex-col" style={{ minHeight: 360 }}>
    ...
    <div className="flex-1 overflow-y-auto px-4 py-3 space-y-4 max-h-[320px]">
      {COMMENTS.map((c) => (
    ...
    <div className="px-3 py-3 border-t border-border/60 flex items-center gap-2">
      <img src={avatarMe} alt="Я" className="w-8 h-8 rounded-full object-cover" />
...
const SearchGrid = ({ onOpen }: { onOpen: (clip: Clip) => void }) => (
  <div className="vk-card rounded-xl p-4">
...
const EmptyState = ({ title }: { title: string }) => (
  <div className="vk-card rounded-xl flex flex-col items-center justify-center py-24 text-center">
...
const RightTabs = ({ tab, setTab, onPublish }: { tab: Tab; setTab: (t: Tab) => void; onPublish: () => void }) => (
  <div className="vk-card rounded-xl p-2">
...
const PublishModal = ({ open, onClose }: { open: boolean; onClose: () => void }) => (
  <Dialog open={open} onOpenChange={(v) => !v && onClose()}>
    <DialogContent className="max-w-[480px] rounded-2xl border-border bg-popover p-8 text-center">
